# TG × Media Backup — Android (Native Kotlin) `v3.1.3`
...
- **🔁 Reproducible Builds**: Disabled Gradle's VCS-info and dependency-metadata embedding (`vcsInfo.include = false`, `dependenciesInfo.includeInApk = false`) so the APK built from source matches the release binary byte-for-byte — required for F-Droid's build verification.
