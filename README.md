# TG × D.Paradox — Android (Native Kotlin) `v2.2.7`
...
- **🖼️ Enhanced Media Viewer**: Completely redesigned with smooth pinch-to-zoom (up to 5x) and bounded panning.
- **📄 Media Details**: New info dialog showing file path, size, resolution, and more.
- **🎬 Better Video Support**: Improved play trigger logic and fallback player chooser.
- **🗑️ Smart Deletion**: Physical file deletion now works seamlessly with Android's Scoped Storage and refreshes the UI immediately.
- **❤️ Reactive Favorites**: Heart status now updates instantly across the app via database observation.
- **📦 Database Hashing**: Implemented SHA-256 database verification to eliminate duplicate backups.
- **🔧 Build Stabilization**: Unified toolchain and resolved JDK version conflicts for better developer experience.
