...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Split-upload support: one row per chunk of a large file.
                db.execSQL(
                    """
                    CREATE TABLE IF NOT EXISTS `file_parts` (
                        ...
                        PRIMARY KEY(`hash`, `partIndex`)
                    )
                    """
                )
                db.execSQL(
                    "ALTER TABLE `uploaded_files` ADD COLUMN `isSplit` INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
