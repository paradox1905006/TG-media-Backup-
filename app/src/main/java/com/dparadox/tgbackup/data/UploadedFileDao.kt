...
interface UploadedFileDao {
    ...
    @Query(
        ...
        SELECT
          ...
          SUM(CASE WHEN status = 'success' AND mimeType LIKE 'audio/%' THEN 1 ELSE 0 END) AS music,
    ...
    // Split files have no telegramFileId of their own (bytes live in
    // file_parts instead), so they must be included via isSplit = 1
    // even though telegramFileId is blank for them.
    @Query("SELECT * FROM uploaded_files WHERE (telegramFileId != '' OR isSplit = 1) AND isDownloaded = 0")
