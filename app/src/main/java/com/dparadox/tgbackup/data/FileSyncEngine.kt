...
class FileSyncEngine(private val context: Context) {
    ...
    companion object {
        ...
        // Chunk size used for split-uploading files larger than MAX_UPLOAD_BYTES.
        //
        // FIX: This was 45 MB, which uploads fine (sendDocument allows up to 50 MB),
        //      but Telegram's Bot API enforces a SEPARATE, much smaller limit on
        //      *downloading* files back out via getFile(): any file above 20 MB
        //      returns "file is too big" and getFile() fails outright. That's why
        //      every split/chunked upload appeared to work but could never be
        //      restored — DownloadWorker.restoreSplitFile() would throw on the
        //      very first chunk it tried to download, get caught by the outer
        //      try/catch in DownloadWorker.runDownload(), get logged, and the
        //      record would stay isDownloaded = false forever (retried and
        //      failed identically on every subsequent restore run). Whole,
        //      non-split files under ~20 MB were never affected, which is why
        //      "normal" files always restored fine.
        //      Chunks now stay under the 20 MB download ceiling, with headroom
        //      for multipart/form overhead.
        const val SPLIT_CHUNK_BYTES = 18L * 1024L * 1024L
    }
    ...
    /**
     * Scan the entire device for folders containing AUDIO files (used by the
     * Music screen's folder-level backup selection — kept separate from
     * [getAllFoldersOnDevice], which is photo/video only).
     */
    suspend fun getAllAudioFoldersOnDevice(): List<String> = withContext(Dispatchers.IO) {
        val results = mutableSetOf<String>()
        ...
        for (collection in collections) {
            try {
                ...
                Log.e(TAG, "Error querying audio folders from $collection", e)
            }
        }
    ...
    private fun isMediaExtension(ext: String) = ext in listOf(
        "jpg", "jpeg", "png", "webp", "mp4", "mkv", "mov", "gif",
        // Audio (music backup)
        "mp3", "m4a", "wav", "flac", "ogg", "aac", "opus", "wma"
    )

    /**
     * Scan ALL media (photos + videos, plus audio when [includeAudio] is true)
     * from both internal and external storage. Ignore folder filters and
     * backup everything found.
     *
     * @param onProgress Called after each file is hashed.
     * @param includeAudio When true, music/audio files are scanned alongside
     * photos & videos for the Music Backup feature.
     */
    suspend fun scanAllMedia(
        ...
        // Scan both INTERNAL and EXTERNAL content for Images and Videos,
        // and Audio too when Music Backup is enabled in Settings.
        ...
        fun collectFiles(dir: DocumentFile, currentPath: String) {
            dir.listFiles().forEach {
                if (it.isDirectory) {
                    ...
                    val mime = it.type ?: ""
                    val isMatch = mime.startsWith("image/") || mime.startsWith("video/") ||
    ...
    /**
     * Public hash lookup for a single file (used by the Music screen to compute
     * the same hash the sync worker would use, so a track selected there matches
     * up with [dbLookup]/manual-selection filtering during the real sync pass).
     */
    suspend fun getFileHash(
        uri: Uri,
        size: Long,
        modifiedSeconds: Long,
        dbLookup: (suspend (size: Long, modified: Long) -> String?)? = null
    ): String? = withContext(Dispatchers.IO) {
        dbLookup?.invoke(size, modifiedSeconds) ?: hashFile(uri, size, modifiedSeconds)
    }
