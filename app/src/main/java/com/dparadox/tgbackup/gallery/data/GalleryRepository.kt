...
class GalleryRepository @Inject constructor(
    ...
    fun observeMediaItem(mediaStoreId: Long): Flow<MediaItem?> {
        return mediaItemDao.observeByMediaStoreId(mediaStoreId).map { it?.let { MediaItem.fromEntity(it) } }
    }
    ...
    suspend fun deletePermanently(ids: List<Long>): IntentSender? {
        val items = mediaItemDao.getByMediaStoreIds(ids)
        val uris = items.map { Uri.parse(it.uri) }
        val intentSender = mediaStoreDataSource.deleteMedia(uris)
