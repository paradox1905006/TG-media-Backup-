...
class MediaStoreDataSource @Inject constructor(
    ...
    suspend fun syncMediaStore() = withContext(Dispatchers.IO) {
        ...
        val allIds = all.map { it.mediaStoreId }.toSet()
        val toRemove = existing.filter { it.mediaStoreId !in allIds }.map { it.mediaStoreId }
        if (toRemove.isNotEmpty()) {
            mediaItemDao.deletePermanently(toRemove)
        }
    ...
    suspend fun deleteMedia(uris: List<Uri>): IntentSender? = withContext(Dispatchers.IO) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            try {
                ...
                return@withContext recoverableSecurityException?.userAction?.actionIntent?.intentSender
            }
        } else {
            uris.forEach { context.contentResolver.delete(it, null, null) }
        }
